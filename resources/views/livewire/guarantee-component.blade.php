<div>
    ...
    <x-principal>
        ...
        <div class="px-6 py-4 flex">
            <div class="flex-1">
                @livewire('guarantee-create-component')
            </div>
            <div class="flex-1">
                ...
                @if(count($guaranties))
                    @foreach ($guaranties as $guarantee)
                        <tr class="border-gray-200 border-b-2">
                            <td class="px-6 py-4 text-md text-gray-500">{{ $guarantee->name }}</td>
                            <td class="px-6 py-4 text-md text-gray-500 text-right">
                                <div class="inline-block">
                                    @livewire('guarantee-edit-component',['guarantee'=>$guarantee],key($guarantee->id))
                                </div>
                                <div class="inline-block">
                                    <x-danger-button wire:click="$dispatch('deleteGuarantee', {{ $guarantee->id }})">
    ...
    @push('js')
        <script>
            Livewire.on('deleteGuarantee', guaranteeId => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        Livewire.dispatchTo('guarantee-component','delete', { guaranteeId: guaranteeId})
