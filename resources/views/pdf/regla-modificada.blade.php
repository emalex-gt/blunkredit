    ...
    <body>
        ...
        <table style="width:100%">
            ...
            @php
                ...
                $tot_total=0;             
            @endphp
            @foreach($credit->amortizacion_schedule->where('status','>',1) as $table)
                @if($table->status==4)
                    <tr>
                        <td>{{ $table->number }}</td>
                ...
                @else
                    ...
                    @foreach($table->partialpayments as $partial)
                        <tr>
                            <td>{{ $table->number }}</td>
                            <td style="text-align:center">{{ date('d/m/Y',strtotime($partial->payment_date)) }}</td>
                            <td style="text-align:center">{{ date('d/m/Y',strtotime($table->share_date)) }}</td>
                            <td style="text-align:center">REI</td>
                            <td style="text-align:center">{{ $table->id }}-{{ $partial->id }}</td>
                            <td style="text-align:center">
                                {{ $diff->days }}
                            </td>
                            <td style="text-align:center">{{ number_format($partial->capital,2,'.',',') }}</td>
                            <td style="text-align:center">{{ number_format($partial->interest,2,'.',',') }}</td>
                            <td style="text-align:center">{{ number_format($partial->delay,2,'.',',') }}</td>
                            <td style="text-align:center">{{ number_format($partial->total_payment,2,'.',',') }}</td>
                            <td style="text-align:center">{{ number_format($table->capital_balance,2,'.',',') }}</td>
                            <td style="text-align:center">{{ number_format($table->interest_balance,2,'.',',') }}</td>
                            <td style="text-align:center">{{ number_format($table->total_balance,2,'.',',') }}</td>
                            <td style="text-align:center">{{ $partial->payment_by->id }}</td>
                            <td style="text-align:center">{{ $partial->payment_by->name }}</td>
                            @php
                                $tot_dias=$tot_dias+$diff->days;
                                $tot_capital=$tot_capital+$partial->capital;
                                $tot_interes=$tot_interes+$partial->interest;
                                $tot_mora=$tot_mora+$partial->delay;
                                $tot_total=$tot_total+$partial->total;
                            @endphp
                        </tr>
                    @endforeach
                @endif
        ...
        <table style="width:100%">
            ...
            @php
                $contador_prepayment=0;
            @endphp
            @foreach($credit->amortizacion_schedule->where('status',1) as $table)
                @if(count($table->prepayment) > 0)
                    @foreach($table->prepayment as $prepayment)
                        <tr>
                            <td>{{ $table->number }}</td>
                            <td style="text-align:center">{{ date('d/m/Y',strtotime($prepayment->payment_date)) }}</td>
                            <td style="text-align:center">{{ date('d/m/Y',strtotime($table->share_date)) }}</td>
                            <td style="text-align:center">REI</td>
                            <td style="text-align:center">{{ $table->id }}</td>
                            <td style="text-align:center">
                ...
                @endif
            @endforeach
            @if($contador_prepayment==0)
                <tr><td colspan="15" style="text-align:center">Sin pagos por aplicar</td></tr>
            @endif
            <tr>
        ...
        @if(count($credit->amortizacion_schedule->where('status',1))>0)
            ...
            <center>
                <table style="width:100%">
                    ...
                    <tr>
                        ...
                        <td style="border-left:1px solid #000;text-align:center">{{ number_format($credit->amortizacion_schedule->where('status',1)->first()->capital,2,'.',',') }}</td>
                        <td style="border-left:1px solid #000;text-align:center">{{ number_format($credit->amortizacion_schedule->where('status',1)->first()->interest,2,'.',',') }}</td>
                        <td style="border-left:1px solid #000;text-align:center">{{ number_format($credit->amortizacion_schedule->where('status',1)->first()->delay,2,'.',',') }}</td>
                        <td style="border-right:1px solid #000;border-left:1px solid #000;text-align:center">{{ number_format($credit->amortizacion_schedule->where('status',1)->first()->total,2,'.',',') }}</td>
                    </tr>
                    <tr>
                        <td style="border-bottom:1px solid #000;border-left:1px solid #000;text-align:center">Cancelación Anticipada</td>
                        <td style="border-bottom:1px solid #000;border-left:1px solid #000;text-align:center">{{ number_format($credit->amortizacion_schedule->where('status',1)->first()->capital,2,'.',',') }}</td>
                        <td style="border-bottom:1px solid #000;border-left:1px solid #000;text-align:center">{{ number_format($credit->amortizacion_schedule->where('status',1)->first()->interest,2,'.',',') }}</td>
                        ...
                        <td style="border-bottom:1px solid #000;border-left:1px solid #000;border-right:1px solid #000;text-align:center">
                            @php
                                $tota=$credit->amortizacion_schedule->where('status',1)->first()->total;
                                $delay=$credit->amortizacion_schedule->where('status',1)->first()->delay;
                                $tot=$tota-$delay;
                            @endphp
                            {{ number_format(($tot),2,'.',',') }}
                        </td>
